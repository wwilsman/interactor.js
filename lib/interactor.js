...
const ArrangeCleanupSymbol = Symbol('@@interactor|arrange|cleanup');
...
export class Interactor {
  ...
  /**
   * @template T
   * @param {() => T extends Function | Promise ? T : any} setup
   * @returns {import('./chainable').Chainable<Interaction, Interactor, any>}
   */
  arrange(setup) {
    return this.act(async () => {
      let k = setup.toString();
      let cleanup = this[ArrangeCleanupSymbol][k];
      ...
      this[ArrangeCleanupSymbol][k] = () => {
        delete this[ArrangeCleanupSymbol][k];
        return result();
      };
    });
  }
