...
export class Interactor {
  ...
  constructor(options) {
    ...
    if (this.constructor.assert != null) {
      this.constructor.Assert = class Assert extends this.constructor.Assert {};
      this.constructor.Assert.defineAssertions(this.constructor.assert);
      delete this.constructor.assert;
    }
  }
  ...
  find(selector) {
    let { then: scoped } = chainable(
      Object.defineProperty({}, Interaction.Symbol, {
        ...
        value: function find({ immediate }) {
          let top = immediate.immediate === immediate.i[Context.Symbol];
          if (!top && selector) immediate.set({ immediate: { selector } });
          else if (selector) immediate.set({ selector });
        }
      }), this, interact);
