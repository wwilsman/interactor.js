...
defineProperties(Interactor, {
  ...
  find: {
    get: function() {
      return assign(m.set((selector, properties) => {
        return m.new(this(selector, properties), 'nested', true);
...
assign(Interactor.prototype, {
   ...
   */
  find(selector, properties) {
    let q = m.get(selector, 'queue');
    let i = q ? selector : Interactor(selector, properties);
