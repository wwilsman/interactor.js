...
// Helper to retrieve a normalized key definition from the definitions list below.
export default function getKeyDefinition(key, mods) {
  // invert keys when shift is pressed
  if (key.length === 1 && mods.shiftKey) {
    let upper = key.toUpperCase();
    key = upper === key ? key.toLowerCase() : upper;
  }

  // look up key definition
  let d = definitions[key];
  if (!d) throw error(`unknown key \`${key}\``);

  // normalize the definition shift keys and defaults
  let definition = {
    text: (mods.shiftKey && d.shiftText) || d.text || '',

    event: {
      code: d.code,
      location: d.location || 0,
      key: (mods.shiftKey && d.shiftKey) || d.key,
      keyCode: (mods.shiftKey && d.shiftKeyCode) || d.keyCode
    }
  };

  // single letter keys are used as the text
  if (definition.event.key.length === 1) {
    definition.text = definition.event.key;
  }

  // if any modifiers besides shift are pressed, no text should be sent
  if (mods.altKey || mods.ctrlKey || mods.metaKey) {
    definition.text = '';
  }
