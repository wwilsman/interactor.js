...
export async function exec($element, parsed, range, delay) {
  ...
  for (let i = 0, l = parsed.length; i < l; i++) {
    ...
    let r = range && (i === 0 ? range : range[0] + i);

    // execute keydown and keyup actions
    keydown($element, parsed[i], r);
...
export default function press(keys, { range, delay } = {}) {
  return this
    .assert.exists()
    .exec(async function($element) {
      let parsed = [].concat(keys).map(c => k.parse(this, c));
      await exec($element, parsed, range, delay);
    });
}
